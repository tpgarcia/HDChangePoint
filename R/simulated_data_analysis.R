...
hd.study<-function(simu.data=simu.data, m=30, num.interp=30, n=80,
  ...
  interp.pt <-array(0, dim=c(num.interp, 3, n),
                    dimnames=list(paste("interp",1:num.interp,sep=""),
                                  c("AGE","TMS", "logDiff"), paste("ID",1:n,sep="")))
  ...
  while(( mean.diff > tolerance)&&(iter<itermax)){
    ...
    for(id in 1:length(obsID)){
      ...
      interp.pt[,,id]<-cbind(pseudox[,id], pseudoy[,id], xxstar[,id])

      ## Separate Data frame compare to logT
      subinterpdata<-data.frame(interp.pt[,,id])

      subinterpdata$fac1<-ifelse(subinterpdata$logDiff<0, 1,0)
      subinterpdata$fac2<-1-subinterpdata$fac1
      subinterpdata$fac<-c(rep(1, sum(subinterpdata$fac1)),rep(2, sum(subinterpdata$fac2)))


      pseudo.data[[id]]<-subinterpdata
    ...
    outscam<- scam(TMS~fac1+fac2+s(logDiff,k=10,bs="micx",m=2, by=fac1)+s(logDiff,k=10,bs="micv",m=2, by=fac2),family=gaussian(link="identity"), data=pseud.gen)
    #scam.check(outscam)
  ...
  p<-ggplot(data=aa.gen, aes(x =logDiff, y = TMS, group = person))
   ...
   p<-p+geom_line(color="lightgray", alpha=1)+
    ## spaghetti plot
     stat_smooth(data=aa.gen, method="loess",aes(group=1,colour="blue"), fill="lightblue",  size=1,alpha=0.6)+ #, , se=FALSE, linetype="dashed")
    ## parametric approach
    geom_line(data=population.pred, aes(x =logDiff, y=para.TMS, colour="green"), size=1, alpha=0.6)+
    ...
    geom_line(data=population.pred, aes(x =logDiff, y =TMS, colour="red"), size=1, alpha=0.6)
  ...
  p +geom_vline(aes(xintercept=global.T),  linetype="dashed")+
    scale_colour_manual(name  ="Method",
                        values=c( "blue","green", "red"), labels =c("Lowess","Parametric NLME","Multi-Stage Nonparametric Method"))+
