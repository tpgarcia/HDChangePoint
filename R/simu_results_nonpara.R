...
#'
#' simus.out<-simus.results(nsim=nsim, n=n, res=results, model="logist", num.interp=num.interp, newl=newl, time.length=time.length, bb.true=bb.true,
#'                          b0.true=b0.true, true.sigma.u=true.sigma.u, true.sigma.eps=true.sigma.eps, file=file)
#'
#'
#'
#'
simus.results<-function(nsim=100, n=80, res=results, model="logist", num.interp=45, newl=45, time.length=20, bb.true=0.1, b0.true=0.5,
  ...
  relative.avr.est.Z<-data.frame(Rel.Bias=bias.updated.z/avr.true.infl, Rel.ESD= sd.update.z.all/avr.true.infl, Rel.Boot.SD= avr.boot.sd.Z/avr.true.infl,  Boot.CI=rbind(avr.boot.cp)) #, cp=cp.z)
  ...
  plot(shift.true.xx, avr.true.omega, type="l",  ylim=c(0,1.5), xlim=c(4.2,5.4),
  ...
  plot(shift.true.xx,   avr.true.d2,
       type="l", ylim=c(-6, 6), xlim=c(4.2, 5.4),
