...
#' results<-main.logist.nlme(n=n, model=model, dat=outdat, num.boot=num.boot, true.theta1=true.theta1, true.theta3=true.theta3,
#'                           num.grid=num.grid, eps.sd=eps.sd, dist=dist, para1=para1, para2=para2, para3=para3, para4=para4)
#'
#'
main.logist.nlme<-function(n=80, model="logist", dat=outdat, num.boot=1000,
                           true.theta1=6, true.theta3=1, num.grid=45, eps.sd=0.05, dist="normal", para1=3.5, para2=0.1, para3=0.1, para4=1){ # alpha=-5
  ...
  if ((num.grid %% 2)==0){
    org.time.length<-num.grid/2
  }else{
    org.time.length<-ceiling(num.grid/2)
  }

  ################################
  ## Construct Boostrap dataset ##
  ################################
  new.pred.data<- array(0, dim=c(org.time.length, 5, n),
                        dimnames=list(paste("visit",1:org.time.length,sep=""), c("pseudo.id","pseudo.logS","pseudo.wcov", "true.pseudo.tms", "est.pseudo.tms"), paste("ID",1:n,sep="")));
  ...
  #############################################################
  ## Generate boostrap estimates by generating random errors ##
  #############################################################
  ...
  for(b in 1:num.boot){
    ...
    for(id in 1:n){
      ...
      err<-ind.pred.data[, "true.pseudo.tms"]-ind.pred.data[, "est.pseudo.tms"]
  ...
  return(list( new.pred.data=new.pred.data, est.theta1=est.theta1,
               ...
               sig.eps=sig.eps, #str.sigma.eps=str.sigma.eps, cp_sigma_eps_lower=cp_sigma_eps_lower, cp_sigma_eps_upper=cp_sigma_eps_upper,
