...
# the parametric NLME estimation is performed under the logistic  model (correct)   ##
######################################################################################
...
#'         and the parametric NLME estimation is performed under the correct (logistic) model.
#'
#' @export
#'
...
#'
#'                                 main.logist.nlme(n=80, model="logist", dat=outdat, num.boot=1000,
#'                                 true.theta1=6, true.theta3=1, num.interp=45, eps.sd=0.05, dist="normal", para1=4.3, para2=4.1, para3=0, para4=1)
#'
#'                                 }
#'             stopCluster(cl)
#'
...
main.logist.nlme<-function(n=80, model="logist", dat=outdat, num.boot=1000,
                           true.theta1=6, true.theta3=1, num.interp=45, eps.sd=0.05, dist="normal", para1=3.5, para2=0.1, para3=0.1, para4=1){ # alpha=-5
  ...
  #est.theta<-fixef(tms.nlme)["theta1"]
  #est.beta<-fixef(tms.nlme)["theta2.W.cov"]
  ...
  #######################################################################
  sig.u<-as.numeric(VarCorr(tms.nlme)["theta2.(Intercept)","StdDev"])
  ...
  cp_sigma_u_lower<- as.numeric(cp.varcov$reStruct$subj["sd(theta2.(Intercept))","lower"])
  cp_sigma_u_upper<-as.numeric(cp.varcov$reStruct$subj["sd(theta2.(Intercept))","upper"])
  ...
  for(b in 1:num.boot){
    ...
    boot.tms.nlme<-nlme(boot.tms~logistft(theta1, theta2, theta3,pseudo.logS), fixed=list(theta1~1, theta2~1+pseudo.wcov, theta3~1) , random=list(theta2~1),
                        data=boot.data, groups=~pseudo.id,  start=list(fixed=c(4.5, 4.4,0, 1)), method="ML", verbose=FALSE)
  ...
  return(list( new.pred.data=new.pred.data, est.theta1=est.theta1,
               est.beta=est.beta,est.beta0=est.beta0, est.theta3=est.theta3,  #estimates
               est.str.theta1=est.str.theta1, est.str.beta=est.str.beta,est.str.beta0=est.str.beta0, est.str.theta3=est.str.theta3,#standard errors
               ...
               str.sigma.eps=str.sigma.eps,sig.u=sig.u, cp_sigma_u_lower=cp_sigma_u_lower, cp_sigma_u_upper=cp_sigma_u_upper,
               str.sigma.u=str.sigma.u,  est.rand.ef=est.rand.ef, est.logT=est.logT, true.logT=true.logT,
               ## bias of derivatives
...
# where true data are generated by logistic Model           #####
#################################################################
...
#'         logistic model and the estimation is performed under the logistic model.
#' @export
#'
#' @examples  simus.logist.out<-logit.results(nsim=100, n=80, res=results, theta1=6, theta3=1, beta=0.1, beta0=0.5, num.interp=45, file="nlme_logist")
#'
#'
#'
logit.results<-function(nsim=100, n=80, res=results, theta1=6, theta3=1, beta=0.1, beta0=0.5, num.interp=45, file="nlme_logist"){
  ...
  return(list(fixed.est=fixed.est,
              glob.logT.est=glob.logT.est, glob.logT.cp=glob.logT.cp # scale.para=scale.para,, avr.sum.abs.error.first.deriv=avr.sum.abs.error.first.deriv
              #avr.sum.abs.error= avr.sum.abs.error, avr.sum.abs.error.second.deriv=avr.sum.abs.error.second.deriv
