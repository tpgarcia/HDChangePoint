...
# True data are generated by the logistic model                                          ##
# the parametric NLME estimation is performed under the arctangent model (misspecified)  ##
###########################################################################################
...
#' @param dist
#'
#' @return Generate simulation results for the parametric NLME approach, where true data are generated by the logistic model
#'         but the parametric NLME estimation is performed under the incorrect (arctangent) model.
#' @export
#'
...
#'
#'                                 main.arctan.missp.nlme(n=80, model="logist",  dat=outdat, num.boot=1000,num.interp=45,
...
#'
#'
main.arctan.missp.nlme<-function(n=80, model="logist",  dat=outdat, num.boot=1000,num.interp=45,
                                 true.theta1=6, true.theta3=1,  para1=1, para2=3.5, para3=0, para4=3, para5=1,  eps.sd=0.05, dist="normal"){#1,0.1,0.1,3,1
  ...
  tms.nlme<-nlme(tms~arctanf(gam1, gam2, gam3, gam4, logS), fixed=list(gam1~1, gam2~1+W.cov, gam3~1, gam4~1),
                 random=gam2~1|subj,
                 data=newgrdata, groups=~subj, start=list(fixed=c(1, 0.5, 0.1, 3.5, 0.5)), method="ML", verbose=FALSE)
  ...
  #### standard deviation of sigma_u's coverage probability
  ...
  ################################################
  #names(cp.all): reStruct :	random effects variance-covariance parameters
  ...
  ##########################################################################
  varcov.sigma<-summary(tms.nlme)$apVar
  str.sigma.u<-sqrt(as.numeric(attr(varcov.sigma,"Pars")["reStruct.subj"])^{2})
  str.sigma.eps<-sqrt(as.numeric(attr(varcov.sigma,"Pars")["lSigma"])^{2})
  ...
  for(b in 1:num.boot){
    ...
    boot.tms.nlme<-nlme(boot.tms~arctanf(gam1, gam2, gam3, gam4, pseudo.logS), fixed=list(gam1~1, gam2~1+pseudo.wcov, gam3~1, gam4~1),
                        ...
                        data=boot.data, groups=~pseudo.id, start=list(fixed=c(1, 0.5, 0.1, 3.5, 0.5)), method="ML", verbose=FALSE)
  ...
  return(list(  new.pred.data= new.pred.data, est.beta=est.beta, est.beta0=est.beta0, #estimates
                ...
                newgrdata=newgrdata, sig.eps=sig.eps, cp_sigma_eps_lower=cp_sigma_eps_lower, cp_sigma_eps_upper=cp_sigma_eps_upper, str.sigma.eps=str.sigma.eps,
                sig.u=sig.u, cp_sigma_u_lower=cp_sigma_u_lower, cp_sigma_u_upper=cp_sigma_u_upper, str.sigma.u=str.sigma.u,
                est.rand.ef=est.rand.ef, est.logT=est.logT, true.logT=true.logT,
                ## bias of derivatives
...
# Misspecified by arctangent model         ##
#############################################
...
#'         logistic model, but the estimation is performed under the arctangent model.
#' @export
#'
#' @examples  simus.missp.arctan.out<-missp.arctan.results(nsim=100, n=80, res=results, num.interp=45,  beta=0.1, beta0=0.5, num.interp=45, file="nlme_arctan_")
#'
#'
#'
missp.arctan.results<-function(nsim=100, n=80, res=results, beta=0.1, beta0=0.5, num.interp=45, file="nlme_arctan_missp_"){
  ...
  return(list( fixed.est=fixed.est,
               ...
               #scale.para=scale.para, # avr.sum.abs.error= avr.sum.abs.error, avr.sum.abs.error.first.deriv=avr.sum.abs.error.first.deriv, avr.sum.abs.error.second.deriv=avr.sum.abs.error.second.deriv,
               glob.logT.cp=glob.logT.cp
